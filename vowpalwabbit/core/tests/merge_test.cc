...
TEST(Merge, MergeSimpleModel)
{
  ...
  EXPECT_EQ(result->initial_weights_config.feature_hash_bits, vw1->initial_weights_config.feature_hash_bits);
  EXPECT_EQ(result->initial_weights_config.feature_hash_bits, vw2->initial_weights_config.feature_hash_bits);
  EXPECT_EQ(result->initial_weights_config.feature_width_bits, vw1->initial_weights_config.feature_width_bits);
  EXPECT_EQ(result->initial_weights_config.feature_width_bits, vw2->initial_weights_config.feature_width_bits);

  const size_t length = static_cast<size_t>(1)
      << (result->initial_weights_config.feature_hash_bits + result->initial_weights_config.feature_width_bits);
